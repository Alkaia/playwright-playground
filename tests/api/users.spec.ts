...
test.describe('Users API Tests', () => {
  ...
  test('should fetch user data successfully', async () => {
    const response = await apiClient.getResponse(`/users/${apiTestData.users.validUserId}`);
    const data = await apiClient.get<User>(`/users/${apiTestData.users.validUserId}`);
  ...
  test('should handle 404 error for non-existent user', async () => {
    const response = await apiClient.getResponse(`/users/${apiTestData.users.invalidUserId}`);
